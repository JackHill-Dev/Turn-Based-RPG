...
void Scene::Draw(SDL_Renderer* rnd)
{
	...
	std::for_each(mLayers.begin(), mLayers.end(), [rnd, &rect, &crop, resolutionScaleX, resolutionScaleY](std::vector<RenderObject*> layer) {
...

		{
			...
			rect.w = obj->GetSheet()->GetCellSize().first * obj->scale * resolutionScaleX;
			rect.h = obj->GetSheet()->GetCellSize().second * obj->scale * resolutionScaleY;
	...
	{
		rect.x = t.pos.first * resolutionScaleX;
		rect.y = t.pos.second * resolutionScaleY;
		rect.w = 90 * resolutionScaleX;
		rect.h = 70 * resolutionScaleY;
