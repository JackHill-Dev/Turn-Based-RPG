...
private:
	...
	int prevScene;										// An int to the previous scene (this is for scenes that can be accessed via different scenes e.g. party viewer)
...
public:
	...
	Interface(bool* brunning,std::map<std::string, RenderObject*>* objP, Player* player, int* currentScene); //This is the constructor, we pass in the pointers to various variables which sets them
	...
	void LoadScene(Scenes scene) ; // Loads scene via it's indexed location in the vector, this now uses an enum Scenes which you can find in Globals.h 
